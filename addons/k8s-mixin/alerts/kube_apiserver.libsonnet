// copied from https://github.com/kubernetes-monitoring/kubernetes-mixin/blob/release-0.10/alerts/kube_apiserver.libsonnet
        ...
        rules: [
          {
            ...
            expr: |||
              apiserver_client_certificate_expiration_seconds_count{%(kubeApiserverSelector)s} > 0 and on(job, %(clusterLabel)s) histogram_quantile(0.01, sum by (job, le, %(clusterLabel)s) (rate(apiserver_client_certificate_expiration_seconds_bucket{%(kubeApiserverSelector)s}[5m]))) < %(certExpirationWarningSeconds)s
            ||| % $._config,
            labels: {
              severity: 'warning',
            },
          ...
          },
          {
            alert: 'KubeClientCertificateExpiration',
            expr: |||
              apiserver_client_certificate_expiration_seconds_count{%(kubeApiserverSelector)s} > 0 and on(job, %(clusterLabel)s) histogram_quantile(0.01, sum by (job, le, %(clusterLabel)s) (rate(apiserver_client_certificate_expiration_seconds_bucket{%(kubeApiserverSelector)s}[5m]))) < %(certExpirationCriticalSeconds)s
            ||| % $._config,
            labels: {
              severity: 'critical',
            },
