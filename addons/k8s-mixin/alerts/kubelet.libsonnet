// copied from https://github.com/kubernetes-monitoring/kubernetes-mixin/blob/release-0.10/alerts/kubelet.libsonnet
    ...
    groups+: [
      {
        ...
        rules: [
          ...
          },
          {
            ...
            expr: |||
              sum(changes(kube_node_status_condition{status="true",condition="Ready"}[15m])) by (node, %(clusterLabel)s) > 2
            ||| % $._config,
            'for': '15m',
            labels: {
              severity: 'warning',
          ...
          },
          {
            ...
            expr: |||
              histogram_quantile(0.99, sum(rate(kubelet_pod_worker_duration_seconds_bucket{%(kubeletSelector)s}[5m])) by (instance, le, %(clusterLabel)s)) * on(instance, %(clusterLabel)s) group_left(node) kubelet_node_name{%(kubeletSelector)s} > 60
            ||| % $._config,
            'for': '15m',
            labels: {
              severity: 'warning',
