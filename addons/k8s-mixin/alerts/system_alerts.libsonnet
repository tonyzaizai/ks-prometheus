// copied from https://github.com/kubernetes-monitoring/kubernetes-mixin/blob/release-0.10/alerts/system_alerts.libsonnet
    ...
    groups+: [
      {
        ...
        rules: [
          ...
          },
          {
            ...
            expr: |||
              (sum(rate(rest_client_requests_total{code=~"5.."}[5m])) by (instance, job, namespace, %(clusterLabel)s)
                /
              sum(rate(rest_client_requests_total[5m])) by (instance, job, namespace, %(clusterLabel)s))
              > 0.01
            ||| % $._config,
            'for': '15m',
            labels: {
